...
A watermark is a text added on an image (cf. fig.1), to indicate a copyright and prevent someone to copy it without the right. Our objective is therefore to erase watermarks from images.\\

We chose to use theoretical results on blind source separation in order to create a program erasing watermarks from images.\\
First, we considered the most simple case of a linear watermark, then we considered watermarks which are not necessarily linear but more realistic. In each case, we assumed that we had a set of three images composed of a target image with a watermark to erase and two copies of another image, one with the same watermark and the other without any. 
...
In conclusion, linear watermarks can easily be dealt with blind source separation, but more complex and common watermarks with non-linear transformations need a complementary treatment to obtain satisfying results.
