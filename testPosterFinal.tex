...
\title{Removing watermarks on images using blind sources separation}
\author{}
\institute{\textsc{BATY L\'eo, BRUNOD-INDRIOGO Luca, NAJJAR Chiheb Eddine, LAURET J\'er\'emy}}
...
We chose to use blind source separation in order to create an efficient watermark-removing program.\\
...
We first considered the simple case of linear watermarks, and then more realistic ones which were not necessarily linear. In each case, we assumed that we had a set of three observees (thus $n=3$) composed of a target image with a watermark to remove and two copies of another image, one with the same watermark and the other without any. 
...
\block{}{
As the modification of the images we obtained that way were not necessarily linear, we couldn't expect to remove the watermark only by using blind source separation. The returned values are nevertheless interesting.
...
- Massoud Babaie-Zadeh, "On Blind Source Separation in Convolutive and Non-Linear mixtures", 20 September 2002 - written during his thesis defended in 2002.\\
